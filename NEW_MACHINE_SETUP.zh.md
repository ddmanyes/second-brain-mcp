...
`command` 指向**本機 venv**，`args` 用 `-m` 啟動 package（**不可直接指向 server.py**，會因 relative import 失敗）：
...
`claude mcp add` 不支援 `-m` flag，需直接編輯 `~/.claude.json`：

```bash
python3 -c "
import json
with open('/Users/<你>/.claude.json', 'r') as f:
    d = json.load(f)
...
with open('/Users/<你>/.claude.json', 'w') as f:
...
| `ImportError: attempted relative import with no known parent package` | `args` 直接指向 `server.py`（script 模式無法解析 relative import） | 改用 `"-m", "mcp_second_brain.server"`（桌面版直接改 JSON；Claude Code 用 Python 腳本編輯 `.claude.json`，因 `claude mcp add` 不支援 `-m`）|
