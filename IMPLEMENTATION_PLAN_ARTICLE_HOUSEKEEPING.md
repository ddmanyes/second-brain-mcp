...
現有 `mcp_second_brain/vault_janitor.py` 不移除。新模組抽出可共用的唯讀檢查邏輯，janitor 可逐步改用同一組函式。規劃時（2026-08-29）排程在 AGENTS.md 仍標為 `.disabled`，因此部署階段必須先確認主機現況，不可直接假設已啟用。
...
- 在 second-brain 與 lcdda 兩個 vault 各呼叫一次 tool，並驗證 lcdda-harvest wrapper，確認 vault isolation、structured output 與唯讀。
...
- 兩個 vault 服務均通過 vault isolation 測試，lcdda-harvest wrapper 維持既有四工具契約。
...
- 已從 `7234342` 建立 `feat/article-housekeeping-audit`，路徑為 `/Users/<user>/git-worktrees/sb-article-housekeeping`。
...
- 依 `NEW_MACHINE_SETUP.md` 改用既有 `/Users/<user>/.venvs/second-brain/bin/python`，並以 `PYTHONPATH="$PWD"` 指向隔離 worktree。
...
- 驗證：`PYTHONPATH="$PWD" /Users/<user>/.venvs/second-brain/bin/python -m pytest tests/test_article_audit.py -q` → `4 passed`。
