...
| 情境 | 路徑 | 由誰建立 |
| ---- | ---- | ---- |
| 文章自動截圖 | `figures/{筆記檔名-kebab}/fig-{NN}.png` | `extract_figures_for`（工具自動） |
| 論文圖表（本地 PDF） | `figures/{paper-short-title-kebab}/fig-{NN}.png` | 手動 `pdftoppm` |
| 專案截圖 | `figures/{project-slug}/fig-{NN}.png` | 手動 |
| 其他 / 暫存 | `figures/misc/` | 手動 |
...
- **自動截圖路徑（2026-06-10 起）**：`extract_figures_for` 直接寫進可見的 `figures/`，資料夾名 = 來源筆記檔名 kebab（例：`2024_Smith_FooBar.md` → `figures/2024-smith-foobar/`），檔名 `fig-00.png` 遞增。Obsidian 看得到、可直接 `![[...]]` 嵌入。
  - （舊版曾寫進隱藏的 `.figures/{md5}/fig_000.png`——若 vault 還有舊 `.figures/`，是歷史資料，可重新 `extract_figures_for` 後刪除。）
